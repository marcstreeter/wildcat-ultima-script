...
def healPet(healthPercent):
    pet = cachedPet()
    if not pet or pet.Hits <= 0:  # dead or missing pet
        return False
    if Player.DistanceTo(pet) <= 11 and Player.Mana > 15 and useMagery and not pet.Poisoned:
        if getHealthPercent(pet) < healthPercent or (getHealthPercent(pet) < 80 and useMageryHighHP):
            ...
            Target.TargetExecute(pet.Serial)
            Misc.Pause(60)
            if Journal.Search("You cannot heal a creature that is already dead!"):
                return False
            Timer.Create("spellTimer", 2000)
            return True
    return False
